...
bridge over the same core, though it currently exposes HEVC and AVC only
(VVC/AV1/VP9/VP8 are C++-only for now).
...
`BS_CAPI_SHARED=ON`). It currently covers **HEVC and AVC** (the other codecs
are C++-only). Usage mirrors the C++ API:
...
`BsAvcHandlers`), a per-NAL `BsReport`, and a collected `BsStructReport`
snapshot. SEI messages are delivered one at a time via `BsSeiCallback`
(payload type + bytes), since SEI is heterogeneous and view-backed. Errors
surface through `bs_get_last_error()`; no exceptions or STL types cross the
boundary.
