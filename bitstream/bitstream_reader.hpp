...
public:
    ...
    explicit BitstreamReader(
        std::span<const byte_type> data) noexcept
        : data_(data)
    {
    }
    ...
    [[nodiscard]]
    constexpr std::span<const byte_type>
    data() const noexcept
    {
        return data_;
    }
     ...
     */
    [[nodiscard]]
    constexpr std::size_t save_position()
        const noexcept
    {
        return bit_position_;
    }
        ...
        std::size_t position)
    {
        if (position > data_.size() * 8) {
            throw std::out_of_range(
                "BitstreamReader: invalid position");
        }
    ...
    std::uint16_t read_be16()
    {
        if (!byte_aligned()) {
            throw std::logic_error(
                "read_be16 requires byte alignment");
        }
    ...
    std::uint32_t read_be32()
    {
        if (!byte_aligned()) {
            throw std::logic_error(
                "read_be32 requires byte alignment");
        }
    ...
    remaining_bytes()
    {
        if (!byte_aligned()) {
            throw std::logic_error(
                "remaining_bytes requires byte alignment");
        }
...
protected:
        ...
        std::size_t count) const
    {
        if (count > bits_remaining()) {
            throw std::out_of_range(
                "BitstreamReader: insufficient bits");
        }
    }


    void require_bits_constexpr_safe(
        std::size_t count) const
    {
        if (count > bits_remaining()) {
            throw std::out_of_range(
...
private:
    ...
    [[nodiscard]]
    std::uint64_t read_bits_runtime_small(
        unsigned count)
    {
        std::uint64_t result = 0;
