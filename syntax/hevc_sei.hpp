...
enum class SeiNalUnitKind : std::uint8_t {
    Prefix,
    Suffix
};
...
struct SeiMessageView {
    ...
    [[nodiscard]]
    constexpr KnownSeiPayloadType known_type()
        const noexcept
    {
        switch (payload_type) {
    ...
    constexpr bool is_type(
        SeiPayloadType type) const noexcept
    {
        return payload_type ==
    ...
    [[nodiscard]]
    constexpr bool is_user_data_registered()
        const noexcept
    {
        return is_type(
    ...
    [[nodiscard]]
    constexpr bool is_user_data_unregistered()
        const noexcept
    {
        return is_type(
    ...
    [[nodiscard]]
    constexpr bool is_mastering_display()
        const noexcept
    {
        return is_type(
    ...
    [[nodiscard]]
    constexpr bool is_content_light_level()
        const noexcept
    {
        return is_type(
    ...
    [[nodiscard]]
    constexpr bool is_picture_timing()
        const noexcept
    {
        return is_type(
            SeiPayloadType::PictureTiming);
    }


    [[nodiscard]]
    constexpr bool is_buffering_period()
        const noexcept
    {
        return is_type(
    ...
    [[nodiscard]]
    constexpr bool is_recovery_point()
        const noexcept
    {
        return is_type(
...
[[nodiscard]]
constexpr bool is_prefix_sei_type(
 ...
 */
[[nodiscard]]
inline SeiExtendedValue decode_sei_extended_value(
    ...
    SeiMessageHeader& header)
{
     ...
     */
    const auto type =
        decode_sei_extended_value(data);
    ...
    UserDataUnregisteredView& result)
{
    ...
    result.user_data_payload =
        message.payload.subspan(16);
    ...
    MasteringDisplayColourVolume& result)
{
    ...
    result.white_point.x =
        read_u16(offset);
    offset += 2;

    result.white_point.y =
        read_u16(offset);
    offset += 2;

    result.max_display_mastering_luminance =
        read_u32(offset);
    offset += 4;

    result.min_display_mastering_luminance =
        read_u32(offset);
    ...
    ContentLightLevelInfo& result)
{
    ...
    result.max_content_light_level =
        read_u16(0);

    result.max_pic_average_light_level =
        read_u16(2);
...
inline void find_sei_messages(
    ...
    std::vector<SeiMessageView>& result)
{
    result.clear();
