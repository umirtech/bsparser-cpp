...
struct ScalingListMatrix {
     ...
     */
    std::array<std::uint8_t, kScalingListMaxCoefficients>
        coefficients{};
...
struct ScalingListData {
    ...
    std::array<
        std::array<
            ScalingListMatrix,
            kScalingListMatrixIds>,
        kScalingListSizeIds> matrices{};


    [[nodiscard]]
    constexpr ScalingListMatrix& matrix(
        std::size_t size_id,
        std::size_t matrix_id) noexcept
    {
    ...
    constexpr const ScalingListMatrix& matrix(
        std::size_t size_id,
        std::size_t matrix_id) const noexcept
    {
        return matrices[size_id][matrix_id];
    }
};
 ...
 */
[[nodiscard]]
constexpr std::size_t scaling_list_coefficient_count(
 ...
 */
[[nodiscard]]
constexpr std::size_t scaling_list_dimension(
    std::size_t size_id) noexcept
{
    switch (size_id) {
    case 0:
 ...
 */
[[nodiscard]]
constexpr bool valid_scaling_list_matrix_id(
    std::size_t size_id,
    std::size_t matrix_id) noexcept
{
    if (size_id >= 4 ||
 ...
 */
[[nodiscard]]
constexpr std::size_t scaling_list_matrix_count(
    std::size_t size_id) noexcept
{
    if (size_id >= 4) {
        return 0;
...
kDefaultScalingList8x8Intra = {
    16, 16, 16, 16, 17, 18, 21, 24,
    16, 16, 16, 16, 17, 18, 21, 24,
    17, 17, 17, 17, 18, 21, 24, 25,
    18, 18, 18, 18, 21, 24, 25, 28,
    18, 18, 18, 18, 24, 25, 28, 30,
    21, 21, 21, 21, 25, 28, 30, 32,
    24, 24, 24, 24, 28, 30, 32, 35,
    25, 25, 25, 25, 30, 32, 35, 36
};
 ...
 */
inline constexpr std::array<std::uint8_t, 64>
kDefaultScalingList8x8Inter = {
    16, 16, 16, 16, 17, 18, 20, 24,
    16, 16, 16, 16, 17, 18, 20, 24,
    17, 17, 17, 17, 18, 20, 24, 25,
    18, 18, 18, 18, 20, 24, 25, 28,
    18, 18, 18, 18, 24, 25, 28, 30,
    20, 20, 20, 20, 25, 28, 30, 32,
    24, 24, 24, 24, 28, 30, 32, 35,
    25, 25, 25, 25, 30, 32, 35, 36
};
...
constexpr std::size_t scaling_list_scan_index(
    std::size_t size_id,
    std::size_t coefficient) noexcept
{
    /*
 ...
 */
inline void initialize_scaling_list_matrix(
    ...
    std::size_t size_id)
{
    ...
    matrix.coefficient_count =
        static_cast<std::uint8_t>(
            scaling_list_coefficient_count(size_id));

    matrix.dc_coef = 8;

    for (std::size_t i = 0;
         i < matrix.coefficient_count;
         ++i) {
 ...
 */
inline void initialize_scaling_list_data(
...
scaling_list_dc_coefficient(
    std::int32_t dc_coef_minus8) noexcept
{
...
scaling_list_prediction_matrix_id(
