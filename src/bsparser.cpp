...
#include <asm/hwcap.h>
#include <sys/auxv.h>
extern "C" size_t bsparser_find_start_code_neon_armv7(const uint8_t* data, size_t size, size_t from, size_t& startCodeSize);
#endif
...
// Returns the position of the start code and writes its size (3 or 4)
// to startCodeSize.
...
__attribute__((target("sse2"))) size_t
find_start_code_sse2(const uint8_t* data, size_t size, size_t from, size_t& startCodeSize)
{
    startCodeSize = 0;
    ...
    while (cursor + 16 <= size)
    {
        ...
        {
            // Scalar resolves the exact candidate and its size.
            return find_start_code_scalar(data, std::min(size, cursor + 16), cursor, startCodeSize);
        }

        // Last candidate that can be completely evaluated in this
        // 16-byte block starts at byte 12.
        cursor += 13;
    }
...
find_start_code_avx2(const uint8_t* data, size_t size, size_t from, size_t& startCodeSize)
{
    startCodeSize = 0;
    ...
    while (cursor + 32 <= size)
    {
        ...
        const uint32_t zeros = static_cast<uint32_t>(_mm256_movemask_epi8(_mm256_cmpeq_epi8(bytes, zero)));

        const uint32_t ones = static_cast<uint32_t>(_mm256_movemask_epi8(_mm256_cmpeq_epi8(bytes, one)));
        ...
        if ((three_byte | four_byte) != 0)
        {
            return find_start_code_scalar(data, std::min(size, cursor + 32), cursor, startCodeSize);
        }

        // Last candidate that can be completely evaluated starts at 28.
        cursor += 29;
    }
...
#endif  // BSPARSER_CAN_BUILD_X86_SIMD

#if BSPARSER_HAS_NEON

size_t find_start_code_neon(const uint8_t* data, size_t size, size_t from, size_t& startCodeSize)
{
    startCodeSize = 0;
    ...
    while (cursor + 16 <= size)
    {
        ...
        /*
         * This is only a zero-byte filter.
         *
         * If the block contains no zero bytes, it cannot contain either:
         *
         *   00 00 01
         *   00 00 00 01
         *
         * If it does contain a zero, let the scalar implementation resolve
         * the exact candidate and determine whether it is 3 or 4 bytes.
         */
...
size_t find_start_code_fast(const uint8_t* data, size_t size, size_t from, size_t& startCodeSize)
{
    startCodeSize = 0;
    ...
    switch (backend)
    {
        ...
        case ScanBackend::Avx2:
            return find_start_code_avx2(data, size, from, startCodeSize);

        case ScanBackend::Sse2:
            return find_start_code_sse2(data, size, from, startCodeSize);
        ...
        case ScanBackend::Neon:
            return find_start_code_neon(data, size, from, startCodeSize);
        ...
        case ScanBackend::Neon:
            return bsparser_find_start_code_neon_armv7(data, size, from, startCodeSize);

#endif

        default:
            return find_start_code_scalar(data, size, from, startCodeSize);
    }
}

size_t legacy_start_code(const uint8_t* data, size_t size, size_t from, size_t& startCodeSize)
{
    return find_start_code_fast(data, size, from, startCodeSize);
}
...
Header parse_hevc(const Bytes& d, uint64_t off, bool vvc = false)
{
    ...
    if (vvc)
    {
        static const char* types[] = {"TRAIL",      "STSA",       "RADL",     "RASL", "RSV_VCL_4", "RSV_VCL_5",
...
size_t start_code(const Bytes& d, size_t from, size_t& startCodeSize)
{
...
Unit UnitScanner::make_annexb_unit(
    const uint8_t* payload, size_t payload_size, uint64_t payload_offset, size_t start_code_size
) const
{
    Unit unit;
...
std::vector<Unit> UnitScanner::scan_annexb(bool end_of_stream)
{
    ...
    while (true)
    {
        ...
        {
            units.push_back(make_annexb_unit(
                pending_.data() + payload_begin, next - payload_begin, pending_offset_ + startCodeSize, startCodeSize
            ));
        }
...
{
    return p && p[0] == 0x00 && p[1] == 0x00 && p[2] == 0x01;
}

bool is_start_code4(const uint8_t* p)
{
    return p && p[0] == 0x00 && p[1] == 0x00 && p[2] == 0x00 && p[3] == 0x01;
}
