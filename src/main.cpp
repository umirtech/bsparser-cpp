...
void write_html_report(const std::string& path, const std::string& input_path,
                       const std::string& format, size_t input_size,
...
<style>
:root{color-scheme:dark;--bg:#0b1020;--panel:#151d31;--line:#2e3a56;--text:#e7edf9;--muted:#9daccc;--accent:#63a7ff;--key:#38d39f}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.45 system-ui,sans-serif}header{padding:24px max(24px,calc((100vw - 1500px)/2));background:#0e1628;border-bottom:1px solid var(--line)}h1{margin:0 0 5px;font-size:24px}.muted{color:var(--muted);word-break:break-all}.stats,.chips,.tools{display:flex;flex-wrap:wrap;gap:9px}.stats{margin-top:17px}.card,.chip{background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:9px 12px}.card b{display:block;font-size:19px;color:var(--accent)}.chips{margin-top:12px}.chip{border-radius:999px;padding:3px 9px}main{max-width:1500px;margin:auto;padding:20px 24px}.tools{margin-bottom:13px}input,select,button{font:inherit;color:var(--text);background:var(--panel);border:1px solid var(--line);border-radius:6px;padding:8px 10px}input{min-width:270px}button{cursor:pointer}button:hover{border-color:var(--accent)}.grid{display:grid;grid-template-columns:minmax(0,1fr) 360px;gap:16px}.box{background:var(--panel);border:1px solid var(--line);border-radius:8px;overflow:hidden}table{width:100%;border-collapse:collapse}th{position:sticky;top:0;background:#18223a;color:var(--muted);text-align:left}th,td{padding:8px 10px;border-bottom:1px solid var(--line);white-space:nowrap}tbody tr{cursor:pointer}tbody tr:hover,tbody tr.active{background:#243454}.type{color:#cfbaff}.key{color:var(--key);font-weight:700}.detail{padding:16px;position:sticky;top:12px;max-height:calc(100vh - 120px);overflow:auto}.detail h2{margin-top:0}dt{color:var(--muted);margin-top:10px}dd{margin:1px 0;overflow-wrap:anywhere}.empty{padding:16px;color:var(--muted)}@media(max-width:900px){.grid{grid-template-columns:1fr}.detail{position:static;max-height:none}}
  ...
  output << R"(</div></header><main><div class="tools"><input id="q" placeholder="Filter type, field, value, or offset"><select id="type"><option value="">All types</option></select><button id="keys">Keyframes only</button><button id="download">Download JSON</button></div><div class="grid"><section class="box"><table><thead><tr><th>#</th><th>Offset (Dec / Hex)</th><th>Length</th><th>Type</th><th>Key</th><th>Summary</th></tr></thead><tbody id="rows"></tbody></table><div id="empty" class="empty" hidden>No matching units.</div></section><aside class="box detail"><h2 id="title">Select a unit</h2><div id="detail" class="muted">Click a row to inspect parsed syntax fields.</div></aside></div></main>
<script>
const headers=)"
         << json_array(headers) << R"(;
...
function details(i){selected=i;const h=headers[i],f=h.fields||{};const hexAddr='0x'+h.offset.toString(16).padStart(8,'0');document.querySelector('#title').textContent=`#${i} | ${h.type}`;let s=`<dl><dt>Frame # / Unit #</dt><dd>${i}</dd><dt>Offset (Dec / Hex)</dt><dd>${h.offset} (${hexAddr})</dd><dt>Length</dt><dd>${h.length} bytes</dd><dt>Keyframe</dt><dd>${h.keyframe?'yes':'no'}</dd>`;for(const[k,v]of Object.entries(f))s+=`<dt>${esc(k)}</dt><dd>${esc(v)}</dd>`;document.querySelector('#detail').innerHTML=s+'</dl>';render();}
function render(){const out=[];headers.forEach((h,i)=>{if(!matches(h))return;const hexAddr='0x'+h.offset.toString(16).padStart(8,'0');out.push(`<tr class="${i===selected?'active':''}" data-i="${i}"><td>${i}</td><td>${h.offset} <span class="muted">(${hexAddr})</span></td><td>${h.length}</td><td class="type">${esc(h.type)}</td><td class="${h.keyframe?'key':''}">${h.keyframe?'KEY':''}</td><td>${esc(summary(h))}</td></tr>`)});rows.innerHTML=out.join('');empty.hidden=out.length>0;rows.querySelectorAll('tr').forEach(r=>r.onclick=()=>details(+r.dataset.i));}
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (format == "ivf") {
      ...
      if (codec == bsparser::Codec::Unknown)
        throw std::invalid_argument("unknown codec: " + format);
    ...
    if (!html_path.empty()) {
      write_html_report(html_path, input_path, format, data.size(), headers);
      std::cerr << "HTML report written to " << html_path << '\n';
    }
    for (const auto& header : headers) std::cout << bsparser::to_json(header) << '\n';
