...
static std::size_t scalar_count(const std::uint8_t* d, std::size_t n)
{
    ...
    for (std::size_t i = 0; i + 3 <= n; ++i) {
        if (d[i] == 0 && d[i+1] == 0 && d[i+2] == 1) { ++c; i += 2; }
    }
    return c;
}
...
int main()
{
    ...
    std::size_t sc = 0;
    for (std::size_t k = 0; k < iters; ++k) sc += scalar_count(d, n);
    auto t1 = std::chrono::steady_clock::now();
    double us_s = std::chrono::duration<double, std::micro>(t1 - t0).count();

    auto t2 = std::chrono::steady_clock::now();
    std::size_t sm = 0;
    for (std::size_t k = 0; k < iters; ++k) sm += simd_count(d, n);
    auto t3 = std::chrono::steady_clock::now();
    double us_v = std::chrono::duration<double, std::micro>(t3 - t2).count();

    printf("stream=%zu bytes\n", buf.size());
    printf("scalar : %8.1f us/iter  %7.1f MB/s  (nals=%zu)\n",
           us_s / iters, bytes / (us_s * 1e-6) / 1e6, sc / iters);
    printf("simd   : %8.1f us/iter  %7.1f MB/s  (nals=%zu)\n",
           us_v / iters, bytes / (us_v * 1e-6) / 1e6, sm / iters);
    printf("speedup: %.1fx\n", us_s / us_v);
    return 0;
}
