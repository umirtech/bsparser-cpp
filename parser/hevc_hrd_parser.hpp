...
[[nodiscard]]
constexpr bool valid_hrd_sub_layers(
    std::uint8_t max_num_sub_layers_minus1) noexcept
{
...
[[nodiscard]]
constexpr bool valid_cpb_count(
    std::uint32_t cpb_cnt_minus1) noexcept
{
 ...
 * them.
 */
inline void parse_hrd_common_info(
    RbspBitstreamReader& bs,
    HrdCommonInfo& common)
{
     ...
     */
    common.nal_hrd_parameters_present_flag =
        bs.read_bit();
     ...
     */
    common.vcl_hrd_parameters_present_flag =
        bs.read_bit();
    ...
    if (!common.nal_hrd_parameters_present_flag &&
        ...
        common.sub_pic_cpb_params_in_pic_timing_sei_flag =
            false;
    ...
    common.sub_pic_hrd_params_present_flag =
        bs.read_bit();
    ...
    if (common.sub_pic_hrd_params_present_flag) {
         ...
         * u(5)
         */
        common.dpb_output_delay_du_length_minus1 =
            static_cast<std::uint8_t>(
                bs.read_bits(5));
    ...
    HrdSubLayer& sub_layer)
{
     ...
     * -------------------------------------------------------
     */
    sub_layer.fixed_pic_rate_general_flag =
        bs.read_bit();
    ...
    if (!sub_layer.fixed_pic_rate_general_flag) {
        ...
        sub_layer.fixed_pic_rate_within_cvs_flag =
            bs.read_bit();
    ...
    if (sub_layer.fixed_pic_rate_within_cvs_flag) {
        ...
        sub_layer.elemental_duration_in_tc_minus1 =
            bs.read_ue();
    ...
    if (!sub_layer.low_delay_hrd_flag) {
        ...
        if (!valid_cpb_count(value)) {
            throw std::runtime_error(
                "HRD: cpb_cnt_minus1 outside 0..31");
        }


        sub_layer.cpb_cnt_minus1 =
            static_cast<std::uint8_t>(
                value);
...
inline HrdParseResult parse_hrd_parameters(
    ...
    HrdParameters& hrd)
{
    if (!valid_hrd_sub_layers(
            max_num_sub_layers_minus1)) {

        throw std::invalid_argument(
            "HRD: MaxNumSubLayersMinus1 > 7");
    }
    ...
    hrd.common_info_present =
        common_inf_present_flag;


    hrd.max_num_sub_layers_minus1 =
        max_num_sub_layers_minus1;
 ...
 */
[[nodiscard]]
inline bool validate_hrd_parameters(
    const HrdParameters& hrd) noexcept
{
    ...
    for (std::size_t i = 0;
        ...
        const auto expected =
            static_cast<std::size_t>(
                layer.cpb_cnt_minus1) + 1;


        if (hrd.common
                .nal_hrd_parameters_present_flag) {

            if (layer.nal.cpb_entries.size() !=
                expected) {
                return false;
            }
        }
...
nal_cpb_bit_rate(
    ...
    std::size_t sub_layer,
    std::size_t cpb_index)
{
    if (!hrd.common
            .nal_hrd_parameters_present_flag) {

        throw std::logic_error(
            "HRD: NAL HRD parameters are absent");
    }


    if (sub_layer >= hrd.sub_layer_count()) {
        throw std::out_of_range(
            "HRD: invalid sub-layer");
    }


    const auto& entries =
        hrd.sub_layers[sub_layer]
            .nal.cpb_entries;


    if (cpb_index >= entries.size()) {
        throw std::out_of_range(
            "HRD: invalid CPB index");
    }
    ...
    std::size_t cpb_index)
{
    if (!hrd.common
        ...
        throw std::logic_error(
            "HRD: VCL HRD parameters are absent");
    }
