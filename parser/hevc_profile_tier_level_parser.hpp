...
inline constexpr std::uint8_t
    kMaxProfileTierLevelSubLayers = 8;
...
inline void parse_general_profile(
    ...
    ProfileTierLevel& ptl)
{
     ...
     * Preserve it exactly.
     */
    ptl.general_reserved_zero_44bits =
        bs.read_bits(44);
}
    ...
    std::size_t sub_layer)
{
    if (sub_layer >= ptl.sub_layer.size()) {
        throw std::out_of_range(
            "profile_tier_level: invalid sub-layer");
    }

    auto& layer =
        ptl.sub_layer[sub_layer];
    ...
    for (std::size_t j = 0; j < 32; ++j) {
        layer.profile_compatibility_flag[j] =
            bs.read_bit();
    }
     ...
     * Constraint flags.
     */
    layer.progressive_source_flag =
        bs.read_bit();

    layer.interlaced_source_flag =
        bs.read_bit();

    layer.non_packed_constraint_flag =
        bs.read_bit();

    layer.frame_only_constraint_flag =
        bs.read_bit();
...
inline ProfileTierLevelParseResult
    ...
    ProfileTierLevel& ptl)
{
     ...
     */
    return ProfileTierLevelParseResult{
        true,
        max_num_sub_layers_minus1,
...
[[nodiscard]]
inline bool validate_profile_tier_level(
    const ProfileTierLevel& ptl) noexcept
{
    if (ptl.max_sub_layers_minus1 >=
        kMaxProfileTierLevelSubLayers) {
        return false;
    }
...
[[nodiscard]]
inline bool profile_compatible(
    const ProfileTierLevel& ptl,
    std::size_t profile_idc) noexcept
{
    if (profile_idc >=
        ptl.general_profile_compatibility_flag.size()) {
        return false;
    }
 ...
 */
[[nodiscard]]
inline bool profile_or_compatible(
    const ProfileTierLevel& ptl,
    std::size_t profile_idc) noexcept
{
    if (ptl.general_profile_idc ==
 ...
 */
[[nodiscard]]
constexpr bool is_valid_level_idc(
    std::uint8_t level_idc) noexcept
{
    switch (level_idc) {
...
[[nodiscard]]
constexpr bool is_progressive_only(
    const ProfileTierLevel& ptl) noexcept
{
    return
        ptl.general_progressive_source_flag &&
