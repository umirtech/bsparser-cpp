    ...
    NalUnitSpan bytes)
{
    ...
    const NalUnitHeader header =
        unpack_nal_unit_header(raw_header);
    ...
    if (header.forbidden_zero_bit) {
        throw NalUnitParseError(
            "NAL unit: forbidden_zero_bit is non-zero");
    }
    ...
    if (!header.valid_temporal_id()) {
        throw NalUnitParseError(
            "NAL unit: invalid nuh_temporal_id_plus1");
    }
