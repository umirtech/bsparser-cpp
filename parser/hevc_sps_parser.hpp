...
inline constexpr std::uint8_t
    kSpsMaxSubLayersMinus1 = 7;

inline constexpr std::uint32_t
    kSpsMaxSequenceParameterSetId = 15;
    ...
    if (!matrix.pred_mode_flag) {
        ...
        if (matrix.pred_matrix_id_delta >
            ...
            throw std::runtime_error(
                "SPS: invalid scaling-list prediction matrix id");
        }
 ...
 */
inline void parse_scaling_list_data(
    RbspBitstreamReader& bs,
...
inline void parse_sps_sub_layer_ordering(
    RbspBitstreamReader& bs,
    SequenceParameterSet& sps)
{
    ...
    for (std::size_t i = first_sub_layer;
        ...
        auto& info =
            sps.sub_layer_ordering_info[i];


        /*
         * max_dec_pic_buffering_minus1
         */
        info.max_dec_pic_buffering_minus1 =
            bs.read_ue();
         ...
         */
        info.max_latency_increase_plus1 =
            bs.read_ue();
    }
...
inline void parse_sps_dimensions(
    RbspBitstreamReader& bs,
    SequenceParameterSet& sps)
{
    /*
...
inline void parse_sps_bit_depth_and_poc(
    RbspBitstreamReader& bs,
    SequenceParameterSet& sps)
{
    /*
    ...
    if (sps.log2_max_pic_order_cnt_lsb_minus4 > 12) {
        ...
        throw std::runtime_error(
            "SPS: invalid log2_max_pic_order_cnt_lsb_minus4");
    }
}
...
inline void parse_sps_coding_blocks(
    RbspBitstreamReader& bs,
    SequenceParameterSet& sps)
{
    auto& cb =
     ...
     */
    cb.log2_min_luma_coding_block_size_minus3 =
        bs.read_ue();
     ...
     */
    cb.log2_diff_max_min_luma_coding_block_size =
        bs.read_ue();
     ...
     */
    cb.log2_min_luma_transform_block_size_minus2 =
        bs.read_ue();
     ...
     */
    cb.log2_diff_max_min_luma_transform_block_size =
        bs.read_ue();
     ...
     */
    cb.max_transform_hierarchy_depth_inter =
        bs.read_ue();
     ...
     */
    cb.max_transform_hierarchy_depth_intra =
        bs.read_ue();
}
...
inline void parse_sps_coding_tools(
    RbspBitstreamReader& bs,
    SequenceParameterSet& sps)
{
    /*
 ...
 */
[[nodiscard]]
inline std::vector<DerivedShortTermReference>
derive_rps_references_recursive(
    ...
    std::size_t index)
{
    ...
    if (!rps.inter_ref_pic_set_prediction_flag) {
        ...
        const auto derived =
            derive_explicit_references(rps);
    ...
    if (!rps.inter_prediction.entries.empty()) {
        ...
        const auto& entry =
            rps.inter_prediction.entries.back();
    ...
    std::size_t st_rps_idx)
{
     ...
     */
    prediction.delta_idx_present =
        false;


    prediction.delta_idx_minus1 =
        0;
    ...
    if (st_rps_idx == 0) {
        ...
        throw std::runtime_error(
            "SPS: invalid inter-RPS prediction index");
    }


    prediction.reference_rps_idx =
        static_cast<std::uint32_t>(
            st_rps_idx - 1);
    ...
    sets[st_rps_idx].num_delta_pocs =
        sps_rps_num_delta_pocs(
            sets,
            st_rps_idx);
    ...
    sets[st_rps_idx].num_negative_pics =
        negative;

    sets[st_rps_idx].num_positive_pics =
        positive;
}
 ...
 */
inline void parse_sps_short_term_rps(
    RbspBitstreamReader& bs,
    SequenceParameterSet& sps)
{
    auto& rps =
    ...
    for (std::size_t i = 0;
         i < rps.short_term_ref_pic_sets.size();
         ++i) {

        parse_short_term_ref_pic_set(
            bs,
            rps.short_term_ref_pic_sets,
            i);
    }
}
...
inline void parse_sps_long_term_rps(
    RbspBitstreamReader& bs,
    SequenceParameterSet& sps)
{
    auto& rps =
     ...
     */
    rps.long_term_ref_pics_present_flag =
        bs.read_bit();
    ...
    for (std::size_t i = 0;
         i < rps.num_long_term_ref_pics_sps;
         ++i) {

        /*
         * lt_ref_pic_poc_lsb_sps[i]
         */
        rps.lt_ref_pic_poc_lsb_sps[i] =
            static_cast<std::uint32_t>(
                bs.read_bits(poc_lsb_bits));


        /*
         * used_by_curr_pic_lt_sps_flag[i]
         */
        rps.used_by_curr_pic_lt_sps_flag[i] =
            bs.read_bit();
    }
}
 ...
 */
inline void parse_sps_range_extension(
    RbspBitstreamReader& bs,
    SequenceParameterSet& sps)
{
    auto& extension =
 ...
 */
inline void parse_sps_multilayer_extension(
    RbspBitstreamReader& bs,
    SequenceParameterSet& sps)
{
 ...
 */
inline void parse_sps_3d_extension(
    RbspBitstreamReader& bs,
    SequenceParameterSet& sps)
{
    for (std::size_t i = 0;
         i < sps.three_d_extension.views.size();
         ++i) {

        auto& view =
            sps.three_d_extension.views[i];

        view.iv_di_mc_enabled_flag =
            bs.read_bit();

        view.iv_mv_scal_enabled_flag =
            bs.read_bit();

        if (i == 0) {

            view.log2_ivmc_sub_pb_size_minus3 =
                bs.read_ue();

            view.iv_res_pred_enabled_flag =
                bs.read_bit();

            view.depth_ref_enabled_flag =
                bs.read_bit();

            view.vsp_mc_enabled_flag =
                bs.read_bit();

            view.dbbp_enabled_flag =
                bs.read_bit();

        } else {

            view.tex_mc_enabled_flag =
                bs.read_bit();

            view.log2_ivmc_sub_pb_size_minus3 =
                bs.read_ue();

            view.intra_contour_enabled_flag =
                bs.read_bit();

            view.intra_dc_only_wedge_enabled_flag =
                bs.read_bit();

            view.cqt_cu_part_pred_enabled_flag =
                bs.read_bit();

            view.inter_dc_only_enabled_flag =
                bs.read_bit();

            view.skip_intra_enabled_flag =
                bs.read_bit();
        }
    }
}
 ...
 */
inline void parse_sps_scc_extension(
    RbspBitstreamReader& bs,
    SequenceParameterSet& sps)
{
    auto& extension =
    ...
    if (extension.palette_mode_enabled_flag) {
        ...
        if (extension
            ...
            if (extension
                ...
                throw std::runtime_error(
                    "SPS: too many palette predictor initializers");
            }
...
inline void parse_sps_extension(
    RbspBitstreamReader& bs,
    SequenceParameterSet& sps)
{
    auto& extension =
     ...
     */
    extension.sps_extension_present_flag =
        bs.read_bit();
     ...
     */
    extension.extension_data_present =
        false;
    ...
    SequenceParameterSet& sps)
{
    ...
    if (sps.sps_max_sub_layers_minus1 >
        kSpsMaxSubLayersMinus1) {

        throw std::runtime_error(
            "SPS: invalid sps_max_sub_layers_minus1");
    }
    ...
    if (sps.sps_seq_parameter_set_id >
        kSpsMaxSequenceParameterSetId) {

        throw std::runtime_error(
            "SPS: invalid sps_seq_parameter_set_id");
    }
    ...
    const std::uint32_t chroma_format_idc =
        bs.read_ue();
    ...
    if (sps.pic_width_in_luma_samples == 0 ||
        ...
        throw std::runtime_error(
            "SPS: invalid picture dimensions");
    }
...
[[nodiscard]]
inline SequenceParameterSet parse_sequence_parameter_set(
    RbspBitstreamReader& bs)
{
    SequenceParameterSet sps{};
...
[[nodiscard]]
inline bool validate_sequence_parameter_set(
