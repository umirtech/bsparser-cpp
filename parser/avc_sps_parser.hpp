...
inline constexpr std::uint8_t kDefault8x8Inter[64] = {
     9, 13, 13, 16, 13, 16, 22, 22,
    22, 22, 25, 25, 25, 25, 25, 29,
    ...
    std::uint8_t* list_out)
{
    ...
    for (unsigned j = 0;
        ...
        next_coef =
            (next_coef + delta_scale + 256) % 256;

        list_out[j] =
            static_cast<std::uint8_t>(next_coef);
    }
}
 ...
 */
inline HrdParameters
parse_hrd_parameters(
    RbspBitstreamReader& reader)
{
    HrdParameters hrd;

    hrd.cpb_cnt_minus1 = static_cast<std::uint8_t>(
        read_ue_max(reader, "cpb_cnt_minus1", 31));

    hrd.bit_rate_scale = static_cast<std::uint8_t>(
        reader.read_bits(4));

    hrd.cpb_size_scale = static_cast<std::uint8_t>(
        reader.read_bits(4));

    for (unsigned i = 0;
         i <= hrd.cpb_cnt_minus1;
         ++i) {

        hrd.bit_rate_value_minus1[i] =
            read_ue_max(reader, "bit_rate_value_minus1", 0xFFFFFFFFu);

        hrd.cpb_size_value_minus1[i] =
            read_ue_max(reader, "cpb_size_value_minus1", 0xFFFFFFFFu);
    ...
    hrd.initial_cpb_removal_delay_length_minus1 =
        static_cast<std::uint8_t>(reader.read_bits(5));
    ...
    RbspBitstreamReader& reader)
{
    ...
    if (vui.bitstream_restriction_flag) {
        ...
        vui.max_bytes_per_pic_denom =
            read_ue_max(reader, "max_bytes_per_pic_denom", 16);

        vui.max_bits_per_mb_denom =
            read_ue_max(reader, "max_bits_per_mb_denom", 16);

        vui.log2_max_mv_length_horizontal =
            read_ue_max(reader, "log2_max_mv_length_horizontal", 16);

        vui.log2_max_mv_length_vertical =
            read_ue_max(reader, "log2_max_mv_length_vertical", 16);

        vui.max_num_reorder_frames =
            read_ue_max(reader, "max_num_reorder_frames", 16);

        vui.max_dec_frame_buffering =
            read_ue_max(reader, "max_dec_frame_buffering", 16);
    }
    ...
    RbspBitstreamReader& reader)
{
    ...
    sps.seq_parameter_set_id = static_cast<std::uint8_t>(
        detail::read_ue_max(
            reader,
    ...
    if (is_high_profile(sps.profile_idc)) {
        ...
        sps.chroma_format_idc = static_cast<std::uint8_t>(
            detail::read_ue_max(
                reader,
                "chroma_format_idc",
                3));
        ...
        sps.bit_depth_luma_minus8 = static_cast<std::uint8_t>(
            detail::read_ue_max(
                reader,
                "bit_depth_luma_minus8",
                6));

        sps.bit_depth_chroma_minus8 = static_cast<std::uint8_t>(
            detail::read_ue_max(
                reader,
        ...
        const bool seq_scaling_matrix_present_flag =
            reader.read_bit();
    ...
    sps.log2_max_frame_num_minus4 = static_cast<std::uint8_t>(
        detail::read_ue_max(
            reader,
            "log2_max_frame_num_minus4",
            12));

    sps.pic_order_cnt_type = static_cast<std::uint8_t>(
        detail::read_ue_max(
            reader,
            "pic_order_cnt_type",
            2));
    ...
    if (sps.pic_order_cnt_type == 0) {
        ...
        for (unsigned i = 0;
             i < sps.num_ref_frames_in_pic_order_cnt_cycle;
             ++i) {
    ...
    sps.max_num_ref_frames = static_cast<std::uint8_t>(
        detail::read_ue_max(
            reader,
            "max_num_ref_frames",
            255));
    ...
    if (sps.vui_parameters_present_flag) {
        sps.vui =
            detail::parse_vui_parameters(reader);
    }
...
[[nodiscard]]
inline AvcSpsParseResult
try_parse_sequence_parameter_set(
    RbspBitstreamReader& reader)
{
    try {
...
[[nodiscard]]
inline bool
validate_sequence_parameter_set(
