...
[[nodiscard]]
inline PictureParameterSet parse_picture_parameter_set(RbspBitstreamReader& reader) {
    ...
    pps.num_slice_groups_minus1 =
        static_cast<std::uint8_t>(detail::read_ue_max(reader, "num_slice_groups_minus1", 7));

    if (pps.num_slice_groups_minus1 > 0) {
        auto& map = pps.slice_group_map;

        map.slice_group_map_type =
            static_cast<std::uint8_t>(detail::read_ue_max(reader, "slice_group_map_type", 6));

        const std::uint8_t num_slice_groups = pps.num_slice_groups();
        ...
        switch (map.slice_group_map_type) {
            ...
            case 4:
            case 5:
                map.slice_group_change_direction_flag = reader.read_bit();

                map.slice_group_change_rate_minus1 =
                    detail::read_ue_max(reader, "slice_group_change_rate_minus1", 0xFFFFFu);
                break;

            case 6:
                map.pic_size_in_map_units_minus1 =
                    detail::read_ue_max(reader, "pic_size_in_map_units_minus1", 0xFFFFFu);

                {
                    const std::uint32_t pic_size = map.pic_size_in_map_units_minus1 + 1;

                    const unsigned bits = detail::ceil_log2(pps.num_slice_groups());

                    map.slice_group_id.reserve(pic_size);

                    for (std::uint32_t i = 0; i < pic_size; ++i) {
                        map.slice_group_id.push_back(
                            static_cast<std::uint8_t>(reader.read_bits(bits))
                        );
                    }
                }
                break;
            ...
            default:
                throw ParseError("pic_parameter_set: invalid slice_group_map_type");
        }
    }

    pps.num_ref_idx_l0_default_active_minus1 = static_cast<std::uint8_t>(
        detail::read_ue_max(reader, "num_ref_idx_l0_default_active_minus1", 31)
    );

    pps.num_ref_idx_l1_default_active_minus1 = static_cast<std::uint8_t>(
        detail::read_ue_max(reader, "num_ref_idx_l1_default_active_minus1", 31)
    );

    pps.weighted_pred_flag = reader.read_bit();

    pps.weighted_bipred_idc = static_cast<std::uint8_t>(reader.read_bits(2));

    pps.pic_init_qp_minus26 = detail::read_se_bounded(reader, "pic_init_qp_minus26", 52);

    pps.pic_init_qs_minus26 = detail::read_se_bounded(reader, "pic_init_qs_minus26", 52);

    pps.chroma_qp_index_offset = detail::read_se_bounded(reader, "chroma_qp_index_offset", 52);
    ...
    if (reader.more_rbsp_data()) {
        ...
        pps.second_chroma_qp_index_offset =
            detail::read_se_bounded(reader, "second_chroma_qp_index_offset", 52);
    }
...
[[nodiscard]]
inline AvcPpsParseResult try_parse_picture_parameter_set(RbspBitstreamReader& reader) {
    try {
        return AvcPpsParseResult{parse_picture_parameter_set(reader), true};
