...
inline void parse_vui_aspect_ratio(
    ...
    VuiParameters& vui)
{
     ...
     * u(8)
     */
    vui.aspect_ratio.aspect_ratio_idc =
...
inline void parse_vui_overscan(
    RbspBitstreamReader& bs,
    VuiParameters& vui)
{
...
inline void parse_vui_video_signal_type(
    RbspBitstreamReader& bs,
    VuiParameters& vui)
{
    /*
     ...
     * u(1)
     */
    vui.video_signal.video_full_range_flag =
        bs.read_bit();

    /*
     * colour_description_present_flag
     */
    vui.video_signal.colour.present =
        bs.read_bit();
     ...
     * u(8)
     */
    vui.video_signal.colour.colour_primaries =
        static_cast<std::uint8_t>(
            bs.read_bits(8));
     ...
     * u(8)
     */
    vui.video_signal.colour.transfer_characteristics =
        static_cast<std::uint8_t>(
            bs.read_bits(8));
     ...
     * u(8)
     */
    vui.video_signal.colour.matrix_coefficients =
        static_cast<std::uint8_t>(
            bs.read_bits(8));
}
...
inline void parse_vui_chroma_location(
    RbspBitstreamReader& bs,
    VuiParameters& vui)
{
    vui.chroma_location.present =
        bs.read_bit();
...
inline void parse_vui_default_display_window(
    RbspBitstreamReader& bs,
    VuiParameters& vui)
{
     ...
     */
    vui.default_display_window.left_offset =
        bs.read_ue();

    /*
     * def_disp_win_right_offset
     */
    vui.default_display_window.right_offset =
        bs.read_ue();

    /*
     * def_disp_win_top_offset
     */
    vui.default_display_window.top_offset =
        bs.read_ue();

    /*
     * def_disp_win_bottom_offset
     */
    vui.default_display_window.bottom_offset =
        bs.read_ue();
}
    ...
    std::uint8_t max_num_sub_layers_minus1)
{
     ...
     */
    vui.timing.poc_proportional_to_timing_flag =
        bs.read_bit();
    ...
    if (vui.timing.poc_proportional_to_timing_flag) {
         ...
         * ue(v)
         */
        vui.timing.num_ticks_poc_diff_one_minus1 =
            bs.read_ue();
...
inline void parse_vui_bitstream_restriction(
    RbspBitstreamReader& bs,
    VuiParameters& vui)
{
...
inline VuiParseResult parse_vui_parameters(
    ...
    VuiParameters& vui)
{
    if (max_num_sub_layers_minus1 >=
        kMaxVuiSubLayers) {

        throw std::invalid_argument(
            "VUI: MaxNumSubLayersMinus1 > 7");
    }
     ...
     * -------------------------------------------------------
     */
    parse_vui_default_display_window(
        bs,
        vui);
     ...
     * -------------------------------------------------------
     */
    parse_vui_bitstream_restriction(
        bs,
        vui);
...
[[nodiscard]]
inline bool validate_vui_parameters(
    const VuiParameters& vui) noexcept
{
     ...
     */
    if (vui.aspect_ratio_info_present_flag &&
        vui.aspect_ratio.aspect_ratio_idc == 255) {

        if (vui.aspect_ratio.sar_width == 0 ||
            vui.aspect_ratio.sar_height == 0) {
            return false;
        }
    }
...
[[nodiscard]]
constexpr bool
vui_has_bitstream_restrictions(
    const VuiParameters& vui) noexcept
{
