...
int main(int argc, char** argv) {
    ...
    for (long long iter = 0; iter < iters; ++iter) {
        ...
        switch (rng() % 5) {
        case 0: /* truncate */
            if (!input.empty()) input.resize(rng() % (input.size() + 1));
            break;
        case 1: /* bit flips */
        ...
        case 3: { /* insert random bytes */
            ...
            for (int i = 0; i < n; ++i) junk.push_back(static_cast<std::uint8_t>(rng() % 256));
            const std::size_t pos = input.empty() ? 0 : rng() % (input.size() + 1);
            input.insert(input.begin() + static_cast<std::ptrdiff_t>(pos), junk.begin(), junk.end());
            break;
        }
        case 4: /* prefix + truncation */
            if (!input.empty()) input.resize(rng() % (input.size() + 1));
            if (input.size() < 8) continue;
            break;
        }
