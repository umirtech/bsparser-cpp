...
void print_sps(
    const bs::avc::SequenceParameterSet& sps)
{
    BS_LOG_INFO("  profile_idc: "
        << static_cast<unsigned>(sps.profile_idc)
        << '\n');

    BS_LOG_INFO("  level_idc: "
        << static_cast<unsigned>(sps.level_idc)
        << '\n');

    BS_LOG_INFO("  SPS ID: "
        << static_cast<unsigned>(sps.seq_parameter_set_id)
        << '\n');

    BS_LOG_INFO("  chroma_format_idc: "
        << static_cast<unsigned>(sps.chroma_format_idc)
        << '\n');

    BS_LOG_INFO("  bit_depth_luma: "
        << static_cast<unsigned>(sps.bit_depth_luma_minus8 + 8)
        << '\n');

    BS_LOG_INFO("  log2_max_frame_num: "
        << sps.log2_max_frame_num()
        << '\n');

    BS_LOG_INFO("  pic_order_cnt_type: "
        << static_cast<unsigned>(sps.pic_order_cnt_type)
        << '\n');

    BS_LOG_INFO("  max_num_ref_frames: "
        << static_cast<unsigned>(sps.max_num_ref_frames)
        << '\n');
    ...
    BS_LOG_INFO("  frame_mbs_only_flag: "
        << std::boolalpha
        << sps.frame_mbs_only_flag
        << '\n');

    BS_LOG_INFO("  direct_8x8_inference_flag: "
        << std::boolalpha
        << sps.direct_8x8_inference_flag
        << '\n');

    BS_LOG_INFO("  frame_cropping_flag: "
        << std::boolalpha
        << sps.frame_cropping_flag
        << '\n');

    BS_LOG_INFO("  vui_parameters_present_flag: "
        << std::boolalpha
    ...
    if (sps.vui_parameters_present_flag &&
        ...
        BS_LOG_INFO("  vui.timing: num_units_in_tick="
...
void print_pps(
    const bs::avc::PictureParameterSet& pps)
{
    BS_LOG_INFO("  PPS ID: "
        << static_cast<unsigned>(pps.pic_parameter_set_id)
        << '\n');

    BS_LOG_INFO("  SPS ID: "
        << static_cast<unsigned>(pps.seq_parameter_set_id)
        << '\n');

    BS_LOG_INFO("  entropy_coding_mode_flag: "
        << std::boolalpha
    ...
    BS_LOG_INFO("  weighted_bipred_idc: "
        << static_cast<unsigned>(pps.weighted_bipred_idc)
        << '\n');

    BS_LOG_INFO("  pic_init_qp_minus26: "
        << pps.pic_init_qp_minus26
        << '\n');

    BS_LOG_INFO("  deblocking_filter_control_present_flag: "
        << std::boolalpha
        << pps.deblocking_filter_control_present_flag
        << '\n');

    BS_LOG_INFO("  constrained_intra_pred_flag: "
        << std::boolalpha
...
void print_slice(
    const bs::avc::SliceHeader& header)
{
    BS_LOG_INFO("  first_mb_in_slice: "
        << header.first_mb_in_slice
        << '\n');

    BS_LOG_INFO("  slice_type: "
        << static_cast<unsigned>(header.slice_type)
        << '\n');

    BS_LOG_INFO("  PPS ID: "
        << static_cast<unsigned>(header.pic_parameter_set_id)
        << '\n');

    BS_LOG_INFO("  frame_num: "
        << header.frame_num
        << '\n');
        ...
        [](const avc::NalUnit& nal) {
            ...
            try {
                ...
                BS_LOG_INFO("[SLICE] type="
                    << static_cast<unsigned>(nal.nal_type())
                    << " ref_idc="
