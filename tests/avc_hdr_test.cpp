 ...
 */
void inspect_cll(std::span<const std::uint8_t> payload) {
    if (payload.size() < 4) {
        return;
    }

    g_max_content_light_level =
        (static_cast<std::uint32_t>(payload[0]) << 8) | static_cast<std::uint32_t>(payload[1]);
...
int run_stream(
    ...
    handlers.sps = [](const bs::avc::NalUnit& nal) {
        ...
        auto sps = bs::avc::parse_sequence_parameter_set(reader);

        (void)g_parameter_sets.store_sps(std::move(sps));
    };

    handlers.sei = [](const bs::avc::NalUnit& nal) {
        const auto sei = bs::avc::parse_sei_nal(nal);
    ...
    try {
        (void)bs::avc::dispatch_nals(bytes, bs::NalFramingMode::AnnexB, handlers);
    ...
    for (std::uint8_t id = 0; id < 32; ++id) {
        ...
        check_int("profile_idc", sps->profile_idc, ref, failures);

        check_int("level_idc", sps->level_idc, ref, failures);

        const std::uint32_t coded_width = (sps->pic_width_in_mbs_minus1 + 1) * 16;

        const std::uint32_t coded_height =
            (sps->pic_height_in_map_units_minus1 + 1) * 16 * (sps->frame_mbs_only_flag ? 1u : 2u);

        check_int("coded_width", coded_width, ref, failures);
        check_int("coded_height", coded_height, ref, failures);

        check_int("display_width", sps->pic_width_in_luma_samples(), ref, failures);

        check_int("display_height", sps->pic_height_in_luma_samples(), ref, failures);

        check_int("chroma_format_idc", sps->chroma_format_idc, ref, failures);

        check_int(
            "bit_depth_luma", static_cast<unsigned>(sps->bit_depth_luma_minus8) + 8, ref, failures
        );

        check_int(
            "bit_depth_chroma",
            static_cast<unsigned>(sps->bit_depth_chroma_minus8) + 8,
            ref,
            failures
        );
        ...
        if (sps->vui_parameters_present_flag && sps->vui.video_signal_type_present_flag) {
            check_int(
                "video_full_range_flag", sps->vui.video_full_range_flag ? 1u : 0u, ref, failures
            );

            if (sps->vui.colour_description_present_flag) {
                check_int("colour_primaries", sps->vui.colour_primaries, ref, failures);

                check_int(
                    "transfer_characteristics", sps->vui.transfer_characteristics, ref, failures
                );

                check_int("matrix_coefficients", sps->vui.matrix_coefficients, ref, failures);
            }
        }
    }

    check_int("mastering_display_present", g_mastering_display_present ? 1u : 0u, ref, failures);
