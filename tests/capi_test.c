...
on_hevc_vps(
    void* ctx,
    const BsHevcVideoParameterSet* vps)
{
    (void)ctx;
    (void)vps;
    ++g_hevc_vps;
...
on_hevc_pps(
    void* ctx,
    const BsHevcPictureParameterSet* pps)
{
    (void)ctx;
    (void)pps;
    ++g_hevc_pps;
...
on_avc_sps(
    void* ctx,
    const BsAvcSequenceParameterSet* sps)
{
    (void)ctx;
    (void)sps;
    ++g_avc_sps;
...
on_avc_pps(
    void* ctx,
    const BsAvcPictureParameterSet* pps)
{
    (void)ctx;
    (void)pps;
    ++g_avc_pps;
...
on_hevc_slice(
    void* ctx,
    const BsHevcSliceSegmentHeader* hdr)
{
    (void)ctx;
    (void)hdr;
    ++g_hevc_slices;
...
int main(int argc, char** argv)
{
     ...
     */
    BsReport* probe = bs_parse_report(
        NULL, buf, got, BS_FRAMING_ANNEX_B, 4);
    if (probe == NULL) {
    ...
    long parsed = bs_parse(
        state, buf, got,
        BS_FRAMING_ANNEX_B, 4, &handlers);
    ...
    printf("[c-test] callback dispatch: parsed=%ld vcl=%d sps=%d\n",
    ...
    if (codec == BS_CODEC_HEVC) {
        ...
        bs_parse_hevc(
            tstate, buf, got, BS_FRAMING_ANNEX_B, 4, &th);
    } else {
        BsAvcHandlers th;
        memset(&th, 0, sizeof(th));
        th.sps  = on_avc_sps;
    ...
    printf("[c-test] typed callbacks: hevc vps=%d sps=%d pps=%d sei_msgs=%d (first type=%u) slices=%d | avc sps=%d pps=%d slices=%d\n",
           g_hevc_vps, g_hevc_sps, g_hevc_pps, g_hevc_sei_msgs,
           g_hevc_sei_first_type, g_hevc_slices,
           g_avc_sps, g_avc_pps, g_avc_slices);

    /*
     * Collected struct report (C-side equivalent of bs::StructReport).
     */
    BsStructReport* sr = bs_parse_struct_report(
        NULL, buf, got, BS_FRAMING_ANNEX_B, 4);
    if (sr == NULL) {
    ...
    for (size_t i = 0; i < sr->count; ++i) {
        ...
        case BS_STRUCT_HEVC_VPS:
            printf("  VPS id=%u\n",
                   ((const BsHevcVideoParameterSet*)e->data)
                       ->vps_video_parameter_set_id);
            break;

        case BS_STRUCT_HEVC_SPS:
            printf("  HEVC SPS w=%u h=%u\n",
                   ((const BsHevcSequenceParameterSet*)e->data)
        ...
        case BS_STRUCT_HEVC_PPS:
            printf("  HEVC PPS id=%u\n",
                   ((const BsHevcPictureParameterSet*)e->data)
                       ->pps_pic_parameter_set_id);
            break;

        case BS_STRUCT_AVC_SPS:
            printf("  AVC SPS id=%u\n",
                   ((const BsAvcSequenceParameterSet*)e->data)
                       ->seq_parameter_set_id);
            break;

        case BS_STRUCT_AVC_PPS:
            printf("  AVC PPS id=%u\n",
                   ((const BsAvcPictureParameterSet*)e->data)
     ...
     */
    BsReport* report = bs_parse_report(
        NULL, buf, got,
    ...
    printf("[c-test] report: codec=%d nals=%zu vcl=%zu\n",
           (int)report->codec,
           report->nal_count,
           report->vcl_count);
    ...
    for (size_t i = 0; i < report->nal_count; ++i) {
        ...
        printf("  #%zu off=0x%zx type=%s(%d) vcl=%d size=%zu\n",
               e->index, e->offset, e->nal_type_name,
               e->nal_unit_type, e->is_vcl, e->size);
    }
