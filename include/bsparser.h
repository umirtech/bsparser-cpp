...
namespace bsparser {
  ...
  std::vector<Header> parse_unit(Codec codec, const std::vector<uint8_t>& bytes, uint64_t offset,ParserState* parserState);
  ...
  class UnitScanner {
  public:
    explicit UnitScanner(Codec codec,ParserState* parserState);
  ...
  class StreamParser {
  public:
    explicit StreamParser(Codec codec,ParserState* parserState);
  ...
  class IvfParser {
  public:
    explicit IvfParser(ParserState* parserState);
    std::vector<Header> feed(const uint8_t* data, size_t size);
    std::vector<Header> feed(const std::vector<uint8_t>& data);
