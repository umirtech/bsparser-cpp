...
inline ElementaryStream demux_ps(std::span<const uint8_t> data) {
    ...
    while (pos + 4 <= data.size()) {
        if (data[pos]==0x00 && data[pos+1]==0x00 && data[pos+2]==0x01) {
            ...
            if (sid == 0xBA) { // pack header
                ...
                if (pes_end > data.size()) pes_end = data.size();
                ...
                if (payload_off < pes_end) {
                    ...
                    if (!found && payload.size()>=5) {
                        if (payload[0]==0x00 && payload[1]==0x00 && payload[2]==0x01) {
                            ...
                            if (nal_type==7) codec = Codec::Avc;
                            else if ((payload[4]>>1)==32) codec = Codec::Hevc; // 0x40
                        }
                    }
                ...
                pos = pes_end;
                continue;
            } else if (sid == 0xBC || sid == 0xBD || (sid >=0xBE && sid <=0xBF) || sid==0xB9 || sid==0xB8 || sid==0xB7) {
                if (pos+6 > data.size()) break;
                uint16_t len = (uint16_t(data[pos+4])<<8)|data[pos+5];
                pos += 6 + len;
                continue;
    ...
    if (!found || stream.empty()) return out;
