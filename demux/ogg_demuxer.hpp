...
inline bool read_u32_le(std::span<const uint8_t> d, size_t p, uint32_t& out) {
    if (p + 4 > d.size())
        return false;
    out = uint32_t(d[p]) | (uint32_t(d[p + 1]) << 8) | (uint32_t(d[p + 2]) << 16) |
          (uint32_t(d[p + 3]) << 24);
    return true;
}
...
inline bool next_page(std::span<const uint8_t> data, size_t& pos, OggPage& page) {
    if (pos + 27 > data.size())
        return false;
    if (data[pos] != 0x4F || data[pos + 1] != 0x67 || data[pos + 2] != 0x67 ||
        data[pos + 3] != 0x53)
        return false;
    ...
    uint64_t granule = 0;
    for (int i = 0; i < 8; ++i)
        granule |= uint64_t(data[pos + 6 + i]) << (i * 8);
    page.granule = granule;
    uint32_t serial;
    if (!read_u32_le(data, pos + 14, serial))
        return false;
    page.serial = serial;
    uint8_t segments = data[pos + 26];
    if (pos + 27 + segments > data.size())
        return false;
    size_t seg_table = pos + 27;
    size_t data_off = seg_table + segments;
    uint32_t total = 0;
    for (int i = 0; i < segments; ++i)
        total += data[seg_table + i];
    if (data_off + total > data.size())
        return false;
...
inline ElementaryStream demux_ogg(std::span<const uint8_t> data) {
    ...
    if (data.size() < 27 || data[0] != 0x4F || data[1] != 0x67 || data[2] != 0x67 ||
        data[3] != 0x53)
        return out;
    ...
    while (pos < data.size()) {
        detail::OggPage pg;
        size_t save = pos;
        if (!detail::next_page(data, pos, pg))
            break;
    ...
    for (auto& pg : pages) {
        if (!pg.bos)
            continue;
        std::string codec;
        if (pg.data.size() >= 8 && memcmp(pg.data.data(), "OVP80", 5) == 0)
            codec = "VP80";
        else if (pg.data.size() >= 8 && memcmp(pg.data.data(), "OVP90", 5) == 0)
            codec = "VP90";
        else if (pg.data.size() >= 4 && memcmp(pg.data.data(), "\x81\x41\x56\x30", 4) == 0)
            codec = "AV01";
        else if (pg.data.size() >= 8 && memcmp(pg.data.data(), "AV01", 4) == 0)
            codec = "AV01";
        else if (pg.data.size() >= 5 && pg.data[0] == 0x01 && pg.data[1] == 'v' &&
                 pg.data[2] == 'p')
            codec = "VP80";
        else if (pg.data.size() >= 4 && pg.data[0] == 0x01 && pg.data[1] == 'H')
            codec = "HEVC";
        else if (pg.data.size() >= 4 && memcmp(pg.data.data(), "\x01\x42", 2) == 0)
            codec = "AVC";
        if (!codec.empty()) {
            bool found = false;
            for (auto& s : streams)
                if (s.first == pg.serial)
                    found = true;
            if (!found)
                streams.push_back({pg.serial, {codec, true}});
        }
    }
    ...
    for (auto& s : streams)
        if (s.second.is_video) {
            video_serial = s.first;
            video_codec = s.second.codec;
            break;
        }
    if (video_serial == 0) {
        // Fallback: first non-audio serial (assume video)
        if (!pages.empty()) {
            video_serial = pages[0].serial;
            video_codec = "VP80";
        } else
            return out;
    }
    ...
    for (auto& pg : pages) {
        ...
        if (pg.data.empty())
            continue;
        // Ogg packets: segments already reassembled per page, but packets can span pages (continued
        // flag 0x01)
        [[maybe_unused]] bool continued =
            (pg.data.size() >= 27 && false);  // simplified: pages already give reassembled data per
                                              // our next_page (which concatenates segments)
        // For simplicity, treat each page's data as one packet (works for VP8/VP9 where each frame
        // is one packet) More correct: need to handle packet spanning across pages where last
        // segment ==255
        ...
        if (video_codec == "VP80" || video_codec == "VP90") {
            ...
            // OGG VP8 packet: first packet after BOS is header, subsequent are frames with no extra
            // header We treat all as frames
            if (!cur.empty())
